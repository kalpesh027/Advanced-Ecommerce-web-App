...
export const createProduct = async (req, res) => {
  ...
  try {
    ...
    if (req.files && req.files.length > 0) {
      ...
      try {
        const uploadimges = req.files.map(file => {
          return uploadImageOnCloudinary(file.path)
        })
        const result = await Promise.all(uploadimges)

        imageUrl = result.map(data => data.secure_url)
        console.log("imageUrls", imageUrl)
        req.files.forEach(file => fs.unlinkSync(file.path)); // Remove the local file after uploading to Cloudinary
      } catch (uploadError) {
...
export const updateProduct = async (req, res) => {
  ...
  const mpurl = req.files.map(file => file)
  console.log(mpurl)
  ...
  try {
    ...
    if (imageUrl.length > 0) updatedProductData.imageUrl = imageUrl;
    ...
    if (discountedPrice < price) {
      await sendNotification( updatedProduct); 
    }
...
const sendNotification = async (product) => {
  try {
    const message = `The product "${product.title}" is available at a discounted price of ₹${product.discountedPrice}!`;
