...
const updateCouponValue = asyncHandler(async (req, res) => {
    const { id } = req.user;
    const {totalDiscountedPrice, discount, couponValue } = req.body; 
    ...
    try {
        let cart = await Cart.findOne({ user: id });
        if (!cart) {
            return res.status(404).json(new ApiResponse(404, 'Cart not found for this user', null));
        }

        cart.couponPrice = couponValue; 
