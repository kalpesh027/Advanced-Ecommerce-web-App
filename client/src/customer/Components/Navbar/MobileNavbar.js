...
function CategoryNavigation() {
  ...
  return (
<>
  <header className="sticky top-0 left-0 right-0 flex flex-col bg-gradient-to-r from-orange-500 via-yellow-500 to-orange-600 z-50 shadow-lg w-full">
    {/* Logo and Navigation */}
    <div className="flex justify-between items-center px-3 py-1 w-full">
      <div onClick={handleLogoClick} style={{ cursor: "pointer" }}>
        <img src={logo} alt="logo" className="h-7 w-24" />
      </div>
      {/* Wishlist, Cart, and Register Button */}
      <div className="flex items-center space-x-3">
        {/* Wishlist Icon */}
        <div className="relative cursor-pointer" onClick={showwishlist}>
          <img src={heart} alt="wishlist" className="h-5 w-5" />
          {wishlist && wishlist.length > 0 && (
            <span className="absolute top-0 right-0 bg-purple-700 text-white text-xs rounded-full px-1.5 py-0.5">
              {wishlist.length}
            </span>
          )}
        </div>
        ...
        <div className="relative cursor-pointer" onClick={showCart}>
          <img
            src="https://th.bing.com/th?id=OIP.qZ22ZrAVSKLRbjX5qyAgigHaEf&w=321&h=194&c=8&rs=1&qlt=90&o=6&dpr=1.3&pid=3.1&rm=2"
            alt="cart-icon"
            className="h-5 w-5"
          />
        ...
        {!isAuthenticated && (
          <button
            ...
            className="text-orange-700 font-medium px-3 py-1.5 bg-white rounded-md shadow-md hover:bg-orange-50 transition duration-300 text-sm"
          >
            Register
          </button>
        )}
    ...
    <div className="flex items-center justify-between px-3 w-full mt-1 bg-white rounded-lg shadow-md p-1.5">
      <div className="flex items-center cursor-pointer" onClick={() => setIsModalOpen(true)}>
        <img src={map} alt="location-icon" className="h-4 w-4" />
        <div className="ml-2 text-xs">
          <h4 className="font-semibold text-gray-800">
            {selectedLocation ? selectedLocation.pincode : "Select Pincode"}
          </h4>
      ...
      <div className="flex items-center ml-4">
        <div className="timediv-img flex items-center">
          <img src={clock} alt="clock-icon" className="h-4 w-4" />
          <p className="text-xs ml-2 text-gray-600">
            {selectedLocation
              ? `${selectedLocation.day} ${selectedLocation.start} - ${selectedLocation.end}`
              : "N/A"}
          </p>
        </div>
      </div>
    ...
    {/* Search Bar */}
    <form onSubmit={handleSearch} className="flex mb-3 px-3 mt-1 w-full mx-auto">
      <input
        type="text"
        placeholder="Search essentials, groceries, and more..."
        className="bg-white w-full border-2 border-gray-300 rounded-lg p-2 text-xs focus:outline-none focus:ring-2 focus:ring-orange-600"
        onChange={(e) => setSearch(e.target.value)}
        required
        value={search}
      />
      <button
        type="submit"
        className="bg-orange-600 text-white rounded-lg p-2 ml-2 hover:bg-orange-700 transition duration-300"
      >
        <img
          src="https://cdn.builder.io/api/v1/image/assets/TEMP/99f489e1b79d5739e5672ea85b3334c7c95166c556f16f5b9bc5d5475ab92174?apiKey=b18cd05798ae4dd7bd4cecc4acd64b6b&"
          alt="search-icon"
    ...
    <div className="flex justify-between items-center px-3 w-full mx-auto">
      {isAuthenticated && (
        <button
          onClick={() => navigate("/myprofile/profile-information")} // Directly navigate to profile page
          className="bg-transparent text-white font-medium px-3 py-1 border border-white rounded-md hover:bg-white hover:text-orange-600 transition duration-300 ml-auto"
        >
          My Account
        </button>
      )}
    ...
    {/* Dropdown Menu */}
    {hoverDropdown && (
      <div className="absolute bg-white shadow-xl mt-2 border border-gray-200 rounded-md w-48 max-w-xs mx-auto">
        <button
          onClick={() => navigate("/myprofile/profile-information")}
          className="flex items-center px-3 py-1.5 text-gray-800 hover:bg-gray-200 w-full text-left text-xs"
        >
        ...
        <button
          ...
          className="flex items-center px-3 py-1.5 text-gray-800 hover:bg-gray-200 w-full text-left text-xs"
        >
          <BsBox className="mr-2" /> Orders
        </button>
        <button
          onClick={() => navigate("/login")} // Navigate to login on logout
          className="flex items-center px-3 py-1.5 text-gray-800 hover:bg-gray-200 w-full text-left text-xs"
        >
        ...
        <div className="modal-content p-3">
          ...
          <div className="search-results">
            {filteredData.map((result) => (
              <div
                key={result._id}
                className="search-result-item flex items-center p-2 cursor-pointer hover:bg-gray-100"
                onClick={() => handleSelect(result)}
              >
                <img src={greymap} alt="location" className="h-4 w-4 mr-2" />
